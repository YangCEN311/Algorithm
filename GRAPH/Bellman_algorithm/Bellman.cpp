...
int main(){
    ...
    //reset visited and edge_list
    visited.assign(POINT_NUM+1, false);
    for(int i = 0 ; i < ADJ[root].size(); i++){
        edge_list.push_back(ADJ[root][i]);
    }
    ...
    while(edge_list.size() != 0){
        ...
        //we only need to check the cost of the end point
        if(e.cost + cost[e.head] >= cost[e.end]){
            if(visited[e.end] == false){
                for(int i = 0 ; i < ADJ[e.end].size(); i++)
                    edge_list.push_back(ADJ[e.end][i]);
                visited[e.end] = true;
        ...
        //if we find a smaller cost, there is a negative weight cycle in the graph
        else{
